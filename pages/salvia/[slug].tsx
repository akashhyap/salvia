...
// @ts-ignore
const SalviaPage = ({ story }) => {
    // console.log("salvia article detail", story);
...
export async function getStaticProps({ params }: { params: Params }) {
    try {
        ...
        let { data } = await storyblokApi.get(`cdn/stories/salvia/${params.slug}`, {
            version: "draft",
        });
...
export async function getStaticPaths() {
    try {
        ...
        Object.keys(data.links).forEach((linkKey) => {
            ...
            if (slug.startsWith('salvia/')) {
                paths.push({ params: { slug: slug.replace('salvia/', '') } });
            }
        });
